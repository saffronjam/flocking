...
public:
	...
	template<typename NumberType>
	static sf::Vector4<NumberType> Vec4(const sf::Vector4<NumberType> &low, const sf::Vector4<NumberType> &high)
	{
		return Vec4(low.x, low.y, low.z, low.w, high.x, high.y, high.z, high.w);
	}

	template<typename NumberType>
	static sf::Vector4<NumberType> Vec4(NumberType lowX, NumberType lowY,
										NumberType lowZ, NumberType lowW,
										NumberType highX, NumberType highY,
										NumberType highZ, NumberType highW)
	{
		static_assert(std::is_arithmetic<NumberType>::value, "NumberType must be arithmetic type");
		if constexpr ( std::is_integral<NumberType>::value )
		{
	...
	{
		const auto bVec = Vec4(0, 0, 0, randomizeAlpha ? 0 : 255, 255, 255, 255, 255);
