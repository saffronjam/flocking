...
public:
	...
	template <typename T>
	static sf::Vector2<T> Rotate(const sf::Vector2<T> &vector, float angle, const sf::Vector2<T> &around = Null<T>());
	...
	template <typename T>
	static float DistanceSq(const sf::Vector2<T> &u, const sf::Vector2<T> &v);
...
template <typename T>
float VecUtils::DistanceSq(const sf::Vector2<T> &u, const sf::Vector2<T> &v)
{
